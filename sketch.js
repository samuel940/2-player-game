...
var other_sword, other_beam, other_slash, health2, right_edge, left_edge;
...
function setup() {
  ...
  left_edge = createSprite(0,height/4,5,height/2);
  right_edge = createSprite(width,height/4,5,height/2);
  ...
  cooldown = 0;
  cooldown2 = 0;
  cooldownBar = createSprite(width/4 -225,150,20,120);
  cooldownBar.shapeColor = "black";
  cooldown3 = 0;
  cooldownBar3 = createSprite(width/4 -175,150,20,120)
  cooldownBar3.shapeColor = "black";


  other_cooldownBar = createSprite(3*width/4 +175,150,20,120);
  other_cooldownBar.shapeColor = "black";
  other_cooldownBar3 = createSprite(3*width/4 +225,150,20,120);
...
function draw() {
  ...
  player2.collide(left_edge);
  player.collide(right_edge);
  player2.collide(right_edge);
  ...
  if (player2.x < 0 || player2.x > width) {
    if (player2.x < 0) {
      player2.x = 25;
    } else {
      player2.x = width - 25;
  ...
  //text("3",23)
  if(mana < 200/3) {
    text("0",width/4 + 265,110)
  } else if (mana < 400/3) {
    text("1",width/4 + 265,110)
  } else if (mana < 200) {
    text("2",width/4 + 265,110)
  } else if (mana === 200) {
    text("3",width/4 + 265,110)
  }
...
function Player1() {
  ...
  if(cooldown2 > 0){
    cooldownBar2 = createSprite(width/4 -225,210-2*cooldown2,20,4*cooldown2);
    
    cooldownBar2.lifetime = 1;
    cooldown2 = cooldown2 - 1;
  }
  if(cooldown3 > 0){
    cooldownBar4 = createSprite(width/4 -175,210-cooldown3,20,2*cooldown3);
  ...
  if(keyDown("a")) {
    ...
    if (keyDown("shift") && ground.y - player.y <= 41) {
  ...
  if (keyWentDown("t")) {
    if (keyDown("d") && cooldown3 === 0 && cooldown <3 && cooldown2 < 25 && mana >= 400/3 && dodge <= 40) {
      swordSlash();
      cooldown3 = 60;
    } else if (keyDown("a") && cooldown3 === 0 && cooldown <3 && cooldown2 < 25 && mana >= 400/3 && dodge <= 40) {
      swordSlash();
      cooldown3 = 60;
    } else if (keyDown("a") || keyDown("d")) {

    } else if (cooldown2 === 0 && cooldown < 3 && cooldown3 < 55 && mana >= 200/3 && dodge <= 40) {
      swordBeam();
      cooldown2 = 30;
    }
...
function Player2() {
  ...
  if (keyDown("l") && other_dodge === 0 && other_invincibility === -1) {
    other_dodge = 45;
  }
  ...
  if(other_cooldown2 > 0){
    other_cooldownBar2 = createSprite(3*width/4 + 175,210-2*other_cooldown2,20,4*other_cooldown2);
    
    other_cooldownBar2.lifetime = 1;
    other_cooldown2 = other_cooldown2 - 1;
  }
  if(other_cooldown3 > 0){
    other_cooldownBar4 = createSprite(3*width/4 + 225,210-other_cooldown3,20,2*other_cooldown3);
  ...
  if(keyDown(LEFT_ARROW) ) {
    ...
    if (keyDown("n") && ground.y - player2.y <= 41) {
  ...
  if (keyWentDown("k")) {
    if (keyDown(RIGHT_ARROW) && other_cooldown3 === 0 && other_cooldown <3 && other_cooldown2 < 25 && mana2 >= 400/3 && other_dodge <= 40) {
      swordSlash2();
      other_cooldown3 = 60;
    } else if (keyDown(LEFT_ARROW) && other_cooldown3 === 0 && other_cooldown <3 && other_cooldown2 < 25 && mana2 >= 400/3 && other_dodge <= 40) {
      swordSlash2();
      other_cooldown3 = 60;
    } else if (keyDown(LEFT_ARROW) || keyDown(RIGHT_ARROW)) {

    } else if (other_cooldown2 === 0 && other_cooldown < 3 && other_cooldown3 < 55 && mana2 >= 200/3 && other_dodge <= 40) {
      swordBeam2();
      other_cooldown2 = 30;
    }
